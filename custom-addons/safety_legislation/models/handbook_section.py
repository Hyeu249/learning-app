...
class HandbookSection(models.Model):
    ...
    def write(self, vals):
        ...
        find_current_section = ("modified_section_version_id", "=", self.id)
        current_section_id = self.search([find_current_section], limit=1)
        if "content" in vals and current_section_id:
            current_section_id.get_section_old_new_diff()
    ...
    def check_security(self):
        ...
        sudo_editing = self.env.context.get("sudo_editing")

        if not user_has_admin_group and not sudo_editing:
            if self.safety_management_handbook_id or self.handbook_parent_section_id:
                raise ValidationError(
                    "Không được thao tác trực tiếp handbook, vui lòng chỉnh sửa vào bản copy, vui lòng liên hệ Admin!"
                )
    ...
    def edit_current_section(self):
        ...
        if modified_section:
            ...
            self.with_context({"sudo_editing": True}).modified_section_version_id = (
                section_id
            )
