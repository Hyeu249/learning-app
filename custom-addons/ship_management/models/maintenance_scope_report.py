...
class MaintenanceScopeReportTemplate(models.Model):
    ...
    def _get_deadline_days(self):
        ...
        variable_name = (
            CONST_UTILITIES.INTEGER_MAINTENANCE_SCOPE_REPORT_DEADLINE_DAY_COUNT
        )
    ...
    def get_group_names_of_unapproved_quotes(self):
        ...
        groups = [self.get_group_by_xml_id(xml_id) for xml_id in unique_xml_ids]
...
class MaintenanceScopeReport(models.Model):
    ...
    inspection_date = fields.Date(
        "Inspection Date",
        related="maintenance_scope_id.inspection_date",
        store=True,
        readonly=True,
        tracking=True,
        depends=["maintenance_scope_id", "maintenance_scope_id.inspection_date"],
    )
    ...
    technical_incident_id = fields.Integer(
    ...
    def unlink(self):
        technical_incidents = []
        for record in self:
            if record.technical_incident_id:
                technical_incidents.append((record.technical_incident_id, record.id))

        result = super(MaintenanceScopeReport, self).unlink()

        for technical_incident_id, id in technical_incidents:
            model_name = "legis.technical.incident"
            if not technical_incident_id:
                return
            technical_incident_job = self.env[model_name].browse(technical_incident_id)
            if technical_incident_job:
                technical_incident_job.remove_report_by_id(id)
    ...
    def generate_report(self):
        ...
        ms.maintenance_interval_days AS maintenance_scope_maintenance_interval_days,
    ...
    WHERE
        ...
        and 
        ms.maintenance_interval_days is not null
    ...
    GROUP BY
        ...
        ms.maintenance_interval_days, ms.utilization_time, ms.allowed_usage_time,
