...
class InspectionPlan(models.Model):
    ...
    _description = "Inspection plan records"
    ...
    def _create_default_ship_management_inspection_scopes(self):
        self.ensure_one()
    ...
    def _create_default_technical_inspection_scopes(self):
        self.ensure_one()
    ...
    def _get_default_general_prenoti_days(self):
        ...
        variable_name = UTILITIES_CONST.INTEGER_INSPECTION_PLAN_GENERAL_PRENOTI_DAYS
    ...
    def _get_default_speciallist_prenoti_days(self):
        default_value_model = self._get_default_value_model()
        variable_name = UTILITIES_CONST.INTEGER_INSPECTION_PLAN_SPECIALLIST_PRENOTI_DAYS
        return default_value_model._get_default_value_by_variable_name(variable_name)

    def _prepare_for_inspection_date(self):
        last_inspection_plan = self._get_last_month_inspection_plan()
        record = self.search(
            [
        ...
        if record:
            ...
            prenoti_date = record.expected_date - timedelta(days=general_prenoti_days)
            today = fields.Date.today()

            if prenoti_date <= today:
                if last_inspection_plan:
                    last_inspection_plan._backlog_all_unsatisfy_tasks()

                record._create_default_ship_management_inspection_scopes()
                record._create_default_technical_inspection_scopes()
    ...
    def _send_noti_for_people_need_to_prepare_inspection_plan(self):
        self.ensure_one()
        default_value_model = self._get_default_value_model()
        variable_name = UTILITIES_CONST.USERS_INSPECTION_PLAN_NEED_PREPARE
    ...
    def _send_noti_for_people_need_to_fill_in_real_date(self):
        self.ensure_one()
        default_value_model = self._get_default_value_model()
        variable_name = UTILITIES_CONST.USERS_INSPECTION_PLAN_NEED_REAL_DATE
        user_ids = default_value_model._get_default_value_by_variable_name(
            variable_name
        )
    ...
    def _get_last_month_inspection_plan(self):
        model_name = "ship.inspection.plan"
        record = self.env[model_name].search(
            [("approval_status", "=", CONST.APPROVED), ("real_date", "!=", False)],
