...
class FuelInternalReceiving(models.Model):
    ...
    _description = "Fuel Internal Receiving"
    ...
    do_density = fields.Float(
        string="Density D.O (Tỷ trọng 15°C)", help="Density of D.O at 15°C", default=1,digits=(10, 4)
    )
    ...
    fuel_check_ids = fields.One2many(
        "ship.fuel.check", "fuel_internal_receiving_id", string="Fuel Checks"
    )
    ...
    attachment_file = fields.Image(
        string="Attachment File", help="Upload attachment file here."
    )
    ...
    def _get_bunkering_safety_checklist_html(self):
        ...
        variable_name = CONST_UTILITIES.HTML_FUEL_INTERNAL_BUNKERING_SAFETY_CHECKLIST
    ...
    def create(self, vals_list):
        ...
        for record in result:
            record._create_default_fuel_checks()
    ...
    def _get_default_fuel_checks(self):
        default_value_model = self._get_default_value_model()
        variable_name = CONST_UTILITIES.RELATION_FUEL_INTERNAL_FUEL_CHECKS
        return default_value_model._get_default_value_by_variable_name(variable_name)

    def _create_default_fuel_checks(self):
        default_checks = self._get_default_fuel_checks()
        self.env["ship.fuel.check"].create(
            [
                {
                    "name": check.field_1,
                    "check_type": check.field_2,
                    "fuel_internal_receiving_id": self.id,
                }
...
class FuelOilTankCondition(models.Model):
    ...
    fuel_tank_ids = fields.One2many(
        "ship.fuel.tank", "fuel_oil_tank_condition_id", string="Fuel Tanks"
    )
    company_id = fields.Many2one(
        "res.company", required=True, default=lambda self: self.env.company
    )
