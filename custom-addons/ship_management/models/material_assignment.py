...
class MaterialAssignment(models.Model):
    ...
    technical_incident_job_id = fields.Integer(
    ...
    def unlink(self):
        technical_incident_jobs = []
        for record in self:
            technical_incident_jobs.append(
                (record.technical_incident_job_id, record.id)
            )

        result = super(MaterialAssignment, self).unlink()

        for technical_incident_job_id, id in technical_incident_jobs:
            if technical_incident_job_id != False and technical_incident_job_id != 0:
                technical_incident_job = self._get_technical_incident_job_by_id(
                    technical_incident_job_id
                )
                technical_incident_job.remove_assignment_by_id(id)
    ...
    def action_start_use_material(self):
        self.ensure_one()
        self.start_time_of_use = fields.Datetime.now()

    def action_end_use_material(self):
        self.ensure_one()
        self.end_time_of_use = fields.Datetime.now()
