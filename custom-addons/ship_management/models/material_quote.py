...
class MaterialQuote(models.Model):
    ...
    added_to_warehouse = fields.Boolean(
        "Are material entities added to warehouse", readonly=True
    )
    ...
    def write(self, vals):
        self._post_chatter_message_to_related_model_on_write(
            ...
            tracking_fields=["quantity", "material_supplier_quote_id"],
        )
    ...
    def _get_chatter_message_on_write(self, old_values, vals):
        ...
        for field in old_values:
            if old_values[field] != vals[field]:
                if field == "quantity":
                    message_text += (
                        f"Số lượng: {old_values[field]} -> {vals[field]} <br/>"
                    )
    ...
    @api.depends("unit_price", "delivered_quantity")
    ...
    def _get_emails(self):
        emails = [supplier_id.email for supplier_id in self.material_id.supplier_ids]
        return emails
