...
class MaintenanceScopeTemplate(models.Model):
    ...
    is_inspection = fields.Boolean("Inspection", tracking=True)
    ...
    def write(self, vals):
        ...
        if "last_maintenance_date" in vals or "maintenance_interval_days" in vals:
    ...
    def set_next_maintenance_date_by_last_maintenance_date_and_interval_days(self):
        ...
        if self.last_maintenance_date and self.maintenance_interval_days:
            interval_days = timedelta(int(self.maintenance_interval_days) or 0)
            self.next_maintenance_date = self.last_maintenance_date + interval_days
