...
class PaintQuote(models.Model):
    ...
    def write(self, vals):
        self._post_chatter_message_to_related_model_on_write(
            vals,
            "material_paint_quotes_request_id",
            tracking_fields=["quantity_liter", "paint_supplier_quote_id"],
        )
        ...
        for field in old_values:
            if old_values[field] != vals[field]:
                if field == "quantity_liter":
                    message_text += (
                        f"Số lượng (lít) : {old_values[field]} -> {vals[field]} <br/>"
                    )
    ...
    def _get_emails(self):
        emails = [supplier_id.email for supplier_id in self.paint_id.supplier_ids]
        return emails
