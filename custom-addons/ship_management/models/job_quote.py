...
class JobQuoteTemplate(models.Model):
    ...
    _description = "Job quote records"
    ...
    finished_at = fields.Date("Finished at", readonly=True, tracking=True)
    ...
    def handle_job_state(self):
        for record in self:
            ...
            if not record._is_approved():
                ...
                if record.job_state == CONST.UNAPPROVED:
                    message = "Báo giá đã được duyệt, vui long chọn trạng thái khác!"
                    raise ValidationError(message)

                elif record.job_state == CONST.TODO:
                    record.finished_at = None

                elif record.job_state == CONST.WORKING:
                    record.start_using_material()
                    record.finished_at = None
                    ...
                    if record.finished_at:
                        record.end_using_material()
                    else:
                        raise ValidationError("Vui lòng hoàn thành báo giá trước!")

    def start_using_material(self):
        self.ensure_one()
        for material_assignment_id in self.material_assignment_ids:
            if not material_assignment_id.start_time_of_use:
                material_assignment_id.action_start_use_material()

    def end_using_material(self):
        self.ensure_one()
        for material_assignment_id in self.material_assignment_ids:
            material_assignment_id.action_end_use_material()
    ...
    def _get_groups_for_perform_confirm_button(self):
        ...
        variable_name = CONST_UTILITIES.GROUPS_SHIP_JOB_QUOTE_CHANGING_CONFIRM_JOB_STATE
    ...
    def get_day_number_for_changing_valid_implement_date(self):
        default_value_model = self._get_default_value_model()
        variable_name = (
            CONST_UTILITIES.INTEGER_JOB_QUOTE_DAY_COUNT_VALID_FOT_EDITING_IMPLEMENT_DATE
        )
        return default_value_model._get_default_value_by_variable_name(variable_name)

    def _get_deadline_days(self):
        default_value_model = self._get_default_value_model()
        variable_name = CONST_UTILITIES.INTEGER_JOB_QUOTE_DEADLINE_DAY_COUNT
        return default_value_model._get_default_value_by_variable_name(variable_name)
...
class JobQuote(models.Model):
    ...
    inspection_date = fields.Date(
        "Inspection Date",
        related="maintenance_scope_report_id.maintenance_scope_id.inspection_date",
        store=True,
        readonly=True,
        tracking=True,
        depends=[
            "maintenance_scope_report_id",
            "maintenance_scope_report_id.maintenance_scope_id.inspection_date",
        ],
    )
