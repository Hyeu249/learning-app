...
class MaterialPaintQuotesRequest(models.Model):
    ...
    _description = "Request for material and paint quotes records"
    _inherit = ["utilities.approval.status"]
    _check_company_auto = True
    _edit_field_permissions_list = {
        "material_quote_ids": ["utilities.group_ship_captain"],
    }
    ...
    def create_material_quotes_when_it_is_reached_min(self):
        self.ensure_one()
        material_ids = (
            self.env["ship.material"]
            .search([("company_id", "=", self.company_id.id)])
            .filtered(lambda e: e.min_quantity > e.available_quantity)
        )

        for material_id in material_ids:
            if not material_id._is_material_in_material_quotes():
                material_id._create_material_quote_based_on_min_quantity()
    ...
    def unlink(self):
        ...
        result = super(MaterialPaintQuotesRequest, self).unlink()
        ...
        for technical_incident_id, id in technical_incidents:
            ...
            technical_incident_job.remove_request_by_id(id)
    ...
    def _get_deadline_days(self):
        ...
        variable_name = (
            CONST_UTILITIES.INTEGER_MATERIAL_PAINT_QUOTES_REQUEST_DEADLINE_DAY_COUNT
        )
